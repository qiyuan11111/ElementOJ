...
/*
 ...
 v7 增加属性依赖实现
*/
public class DefaultBeanFactory_v7 implements BeanFactory, BeanDefinitionRegistry, Closeable {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(DefaultBeanFactory_v7.class);

    protected Map<String, com.study.spring.beans.BeanDefinition> beanDefintionMap = new ConcurrentHashMap<>(256);
    ...
    @Override
    public void registerBeanDefinition(String beanName, com.study.spring.beans.BeanDefinition beanDefinition)
    ...
    public Class<?> getType(String name) throws Exception{
        com.study.spring.beans.BeanDefinition bd = this.getBeanDefinition(name);
    ...
    @Override
    public com.study.spring.beans.BeanDefinition getBeanDefinition(String beanName) {
    ...
    public <T> T getBean(Class<T> type) throws Exception {
        ...
        if(names != null) {
            if(names.size() == 1){
                ...
                //找Primary
                com.study.spring.beans.BeanDefinition bd = null;
    ...
    protected Object doGetBean(String beanName) throws Exception {
        ...
        com.study.spring.beans.BeanDefinition bd = this.getBeanDefinition(beanName);
    ...
    private Object doCreateInstance(String beanName, com.study.spring.beans.BeanDefinition bd) throws Exception {
        Class<?> type = bd.getBeanClass();
    ...
    // 给入属性依赖
    private void setPropertyDIValues(com.study2.spring.beans.BeanDefinition bd, Object instance) throws Exception {
    ...
    // 构造方法来构造对象
    private Object createInstanceByConstructor(com.study.spring.beans.BeanDefinition bd)
            throws Exception {
        /*
          构造参数依赖注入，这里需要做些什么？
        */
    ...
    private Object[] getConstructorArgumentValues(com.study.spring.beans.BeanDefinition bd) throws Exception {
    ...
    private Constructor<?> determineConstructor(com.study.spring.beans.BeanDefinition bd, Object[] args) throws Exception {
        /*判定构造方法的逻辑应是怎样的？
    ...
    private Method determineFactoryMethod(com.study.spring.beans.BeanDefinition bd, Object[] args, Class<?> type) throws Exception {
       /*判定工厂方法的逻辑同构造方法的判定逻辑
    ...
    // 静态工厂方法
    private Object createInstanceByStaticFactoryMethod(com.study.spring.beans.BeanDefinition bd) throws Exception {
    ...
    // 工厂bean方式来构造对象
    private Object createInstanceByFactoryBean(com.study.spring.beans.BeanDefinition bd) throws Exception {
     ...
     * @throws Exception
     */
    private void doInit(com.study.spring.beans.BeanDefinition bd, Object instance) throws Exception {
        // 执行初始化方法
    ...
    public void close() throws IOException {
        // 执行单例实例的销毁方法
        for (Entry<String, com.study.spring.beans.BeanDefinition> e : this.beanDefintionMap.entrySet()) {

...
public class DefaultBeanFactory implements BeanFactory, BeanDefinitionRegistry, Closeable {
        ...
        if (type != null) {
            if (StringUtils.isBlank(bd.getFactoryMethodName())) {
                ...
                if(method != null){
                    ...
                    type = this.determineFactoryMethod0(type,bd.getFactoryMethodName(),this.determineArgsType(bd.getConstructorArgumentValues())).getReturnType();
                }
            }
        } else {
            // 工厂bean方式来构造对象的
    ...
    private Class<?>[] determineArgsType(List<?> constructorArgumentValues) throws Throwable {
        if(CollectionUtils.isEmpty(constructorArgumentValues)){
            return null;
        }

        Class<?>[] argTypes = new Class[constructorArgumentValues.size()];
        int i = 0;
        Class<?> type = null;
        for(Object arg : constructorArgumentValues){
            if(arg instanceof BeanReference) {
                BeanReference br = (BeanReference) arg;
                if(StringUtils.isNotBlank(br.getBeanName())){
                    type = this.getType(br.getBeanName());
                }
                else {
                    type = br.getType();
                }
    ...
    private Method determineFactoryMethod(BeanDefinition bd, Object[] args, Class<?> type) throws Throwable {
        ...
        * */
        ...
        m = this.determineFactoryMethod0(type,methodName,paramTypes);
    ...
    private Method determineFactoryMethod0(Class<?> type, String methodName, Class[] paramTypes) {
        Method m = null;
        try {
            ...
            for (Method m0 : type.getMethods()) {
                ...
                if (paramterTypes.length == paramTypes.length) {
                    for (int i = 0; i < paramterTypes.length; i++) {
                        if (!paramterTypes[i].isAssignableFrom(paramTypes[i])) {
                            continue outer;
                        }
                    }
